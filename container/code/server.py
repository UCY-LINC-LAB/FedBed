...
clogger = ClientsLogger()

fl.server.Server.fit = Profiler("overall_fit")(fl.server.Server.fit)
fl.server.Server.fit_round = Profiler("fit_round", store_per_run=True)(fl.server.Server.fit_round)
fl.server.Server.evaluate_round = clogger.log_all_clients(fl.server.Server.evaluate_round)
...
if __name__ == "__main__":
    ...
    for metric in history.losses_centralized:
        Profiler.log_metric("round", metric[0])
        Profiler.log_metric("loss", metric[1])
        Profiler.log_metrics("history_losses_centralized.jl")
    for metric in history.losses_distributed:
        Profiler.log_metric("round", metric[0])
        Profiler.log_metric("loss", metric[1])
        Profiler.log_metrics("history_losses_distributed.jl")
    for metric in history.metrics_centralized:
        for round in history.metrics_centralized[metric]:
            ...
            Profiler.log_metrics("history_metrics_centralized.jl")
